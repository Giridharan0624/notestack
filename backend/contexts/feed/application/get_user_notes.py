...
class GetUserPublicNotesUseCase:
    def __init__(self, repository: FeedRepository):
        self.repository = repository

    def execute(self, user_id: str, limit: int = 20, cursor: str | None = None) -> dict:
        last_key = None
        if cursor:
            last_key = json.loads(base64.b64decode(cursor))

        items, next_key = self.repository.find_public_notes_by_user(user_id, limit, last_key)
