...
export default function PublicNoteCard({ note, index = 0, liked, bookmarked, onLike, onBookmark, onShare }: Props) {
  ...
  return (
    <div className={`bg-white rounded-[var(--r)] overflow-hidden up d${Math.min(index+1,12)}`}
      ...
      <Link href={`/feed/notes/${note.noteId}`}>
        <div className="h-36 relative" style={{ background: pick(note.noteId) }}>
          <span className="absolute inset-0 flex items-center justify-center text-white/20 text-[40px] font-extrabold">{note.title[0]?.toUpperCase()}</span>
        </div>
      </Link>
      <div className="p-4">
        ...
        <Link href={`/feed/notes/${note.noteId}`}>
          <h3 className="text-[15px] font-bold truncate mb-0.5 hover:underline">{note.title}</h3>
        </Link>
        {note.description && (
        ...
        <div className="flex items-center gap-2 pt-2.5" style={{ borderTop: "1px solid var(--border)" }}>
          <Link href={`/profile/${note.userId}`} className="flex items-center gap-2 min-w-0">
            <div className="h-5 w-5 rounded-full flex items-center justify-center text-[9px] font-bold text-white shrink-0" style={{ background: pick(note.userId) }}>{(note.authorDisplayName||"S")[0].toUpperCase()}</div>
            <span className="text-[12px] truncate" style={{ color: "var(--fg3)" }}>{note.authorDisplayName || "Student"}</span>
          </Link>
          <div className="flex items-center gap-1.5 ml-auto shrink-0">
            {onLike && (
              <button onClick={(e) => { e.preventDefault(); onLike(); }} className="flex items-center gap-1 text-[12px] px-1.5 py-0.5 rounded-full transition-colors cursor-pointer"
                style={{ color: liked ? "var(--red)" : "var(--fg4)", background: liked ? "#fef2f2" : "transparent" }}>
                {liked ? "❤️" : "🤍"} {note.likeCount || ""}
              </button>
            )}
            {onBookmark && (
              <button onClick={(e) => { e.preventDefault(); onBookmark(); }} className="text-[12px] px-1.5 py-0.5 rounded-full transition-colors cursor-pointer"
                style={{ color: bookmarked ? "var(--blue)" : "var(--fg4)", background: bookmarked ? "var(--pop-light)" : "transparent" }}>
                {bookmarked ? "🔖" : "📑"}
              </button>
            )}
            {onShare && (
              <button onClick={(e) => { e.preventDefault(); onShare(); }} className="text-[12px] px-1.5 py-0.5 rounded-full transition-colors cursor-pointer" style={{ color: "var(--fg4)" }}>
                📤
              </button>
            )}
          </div>
